...
CANMessageGetRx(unsigned char *pucData, unsigned long *pulMsgID)
{
    ...
    //
    usArbReg[0] = CANRegRead(CAN0_BASE + CAN_O_IF2ARB1);
    usArbReg[1] = CANRegRead(CAN0_BASE + CAN_O_IF2ARB2);
    usMsgCtrl = CANRegRead(CAN0_BASE + CAN_O_IF2MCTL);

    //
    // Set the 29 bit version of the Identifier for this message object.
    //
    *pulMsgID = ((usArbReg[1] & CAN_IF1ARB2_ID_M) << 16) | usArbReg[0];
                ...
                unsigned long ulSize)
{
    ...
    // Set the 29 bit version of the Identifier for this message object.
    //
    usArbReg[0] = ulId & CAN_IF1ARB1_ID_M;

    //
    // Mark the message as valid and set the extended ID bit.
    //
    usArbReg[1] = ((ulId >> 16) & CAN_IF1ARB2_ID_M) |
...
//! the boot loader.   If the \e ulSetTiming paramter is 0, the bit timing
        ...
        switch(ulCmd)
        {
            ...
            case LM_API_UPD_SEND_DATA:
            {
                ...
                if(g_ulTransferSize >= ulBytes)
                {
                    ...
                    DecryptData(pucCommandBuffer, ulBytes);
#endif
                    //
                    ...
                    else
                    {
                        ...
                        //
                        for(ulTemp = 0; ulTemp < ulBytes; ulTemp += 4)
                        {
                            //
                            ...
                            HWREG(FLASH_FMD) =
                                *((unsigned long *)&pucCommandBuffer[ulTemp]);
                            HWREG(FLASH_FMC) =
                                FLASH_FMC_WRKEY | FLASH_FMC_WRITE;
                ...
                // first two bytes of the image to allow it to boot.
                //
                if(g_ulTransferSize == 0)
                {
                    ...
                    for(ulTemp = 0; ulTemp < 2; ulTemp++)
                    {
                        ...
                        // Program the next word.
                        //
                        HWREG(FLASH_FMA) = g_ulStartAddress + (ulTemp << 2);
                        HWREG(FLASH_FMD) = g_ulStartValues[ulTemp];
